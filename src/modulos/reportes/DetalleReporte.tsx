...
interface DetalleReporteProps {
  // Si se proporciona un reporte, se usará ese en lugar de cargar uno nuevo
  reporteProp?: Reporte;
  // Si es true, no se usa el componente Layout (útil para usar dentro de otro componente que ya tiene Layout)
  noLayout?: boolean;
}

const DetalleReporte = ({ reporteProp, noLayout = false }: DetalleReporteProps) => {
  const { id } = useParams<{ id: string }>();
  const [reporte, setReporte] = useState<Reporte | null>(reporteProp || null);
  const [loading, setLoading] = useState(!reporteProp);
  const navigate = useNavigate();

  useEffect(() => {
    // Si se proporciona un reporte como prop, usamos ese
    if (reporteProp) {
      setReporte(reporteProp);
      setLoading(false);
      return;
    }
  ...
  if (loading) {
    ...
    return noLayout ? loadingContent : (
  ...
  if (!reporte) {
    const notFoundContent = (
      <div className="text-center py-10">
        ...
        <Button asChild>
          <Link to="/reportes">Volver a la lista</Link>
        </Button>
      </div>
    );

    return noLayout ? notFoundContent : (
  ...
  return noLayout ? content : <Layout titulo="Detalle del Reporte">{content}</Layout>;
};
