 ...
 * Registra en el historial los cambios en activo, tipo o permisos
 * Actualiza los usuarios afectados cuando cambian estos atributos críticos
 * 
 ...
 * @returns Promise con el rol actualizado o null si hubo un error
 */
export const actualizarRolAvanzado = async (
  ...
  try {
    ...
    // 2. Detectar cambios en atributos críticos
    const cambioActivo = 'activo' in rolData && rolData.activo !== rolActual.activo;
    const cambioTipo = 'tipo' in rolData && rolData.tipo !== rolActual.tipo;
    const cambioPermisos = 'permisos' in rolData && JSON.stringify(rolData.permisos) !== JSON.stringify(rolActual.permisos);
    
    const hayCambiosCriticos = cambioActivo || cambioTipo || cambioPermisos;
    ...
    // 4. Registrar cambios en el historial de roles
    if (hayCambiosCriticos) {
      await registrarCambiosEnHistorial(rolActual, rolActualizado, usuario, motivoCambio);
    }

    // 5. Actualizar usuarios si hay cambios críticos
    if (hayCambiosCriticos) {
      const usuariosActualizados = await actualizarUsuariosAfectados(
        rolActual, 
        rolActualizado, 
        usuario, 
        motivoCambio
